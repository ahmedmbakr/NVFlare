...
def save_pretrained_file(model_name, num_classes, path):
    if model_name == 'ssdnet':
        ...
        model = yolonet.YOLOv5.get_pretrained_model(num_classes)
    else:
        raise ValueError(f"Model name {model_name} is not supported. The supported model names are 'ssdnet', 'resnet', or 'yolov5'.")
    torch.save(model.state_dict(), path)
    

if __name__ == '__main__':
    num_classes = 3
    model_name = 'yolov5' # The options are 'ssdnet', 'resnet', or 'yolov5'
